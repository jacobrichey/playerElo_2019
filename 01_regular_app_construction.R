...
RE19 <- fread("data/09-29-2019-mlb-season-pbp-feed.csv")
...
b19Elo <- data.frame(player_id = unique(RE19$Batter_MLB_ID), 
  ...
  mutate(currentElo = ifelse(is.na(currentElo), 
         ...
         topPitchersN = 0, topPitchersRV = 0, botPitchersN = 0, botPitchersRV = 0,
         PA.x = 0) %>%
...
p19Elo <- data.frame(player_id = unique(RE19$Pitcher_MLB_ID),
  ...
  mutate(currentElo = ifelse(is.na(currentElo), 
                             ...
                             ((currentElo - 1000) * 0.85) + 1000),
         preseasonElo = currentElo, avgEloFaced = 0, 
         topBattersN = 0, topBattersRV = 0, botBattersN = 0, botBattersRV = 0,
         BFP.x = 0) %>%
...
for (row in 1:nrow(RE19)) {
  ...
  # Note performance against especially good and bad competition
  if (pitcherElo > 1100) {
    b19Elo$topPitchersN[which(b19Elo$player_id == batterID)] <- 
      b19Elo$topPitchersN[which(b19Elo$player_id == batterID)] + 1
    b19Elo$topPitchersRV[which(b19Elo$player_id == batterID)] <- 
      b19Elo$topPitchersRV[which(b19Elo$player_id == batterID)] + RE19$Runs_Value[row]
  } else if (pitcherElo < 900) {
    b19Elo$botPitchersN[which(b19Elo$player_id == batterID)] <- 
      b19Elo$botPitchersN[which(b19Elo$player_id == batterID)] + 1
    b19Elo$botPitchersRV[which(b19Elo$player_id == batterID)] <- 
      b19Elo$botPitchersRV[which(b19Elo$player_id == batterID)] + RE19$Runs_Value[row]
  }
  if (batterElo > 1100) {
    p19Elo$topBattersN[which(p19Elo$player_id == pitcherID)] <- 
      p19Elo$topBattersN[which(p19Elo$player_id == pitcherID)] + 1
    p19Elo$topBattersRV[which(p19Elo$player_id == pitcherID)] <- 
      p19Elo$topBattersRV[which(p19Elo$player_id == pitcherID)] + RE19$Runs_Value[row]
  } else if (batterElo < 900) {
    p19Elo$botBattersN[which(p19Elo$player_id == pitcherID)] <- 
      p19Elo$botBattersN[which(p19Elo$player_id == pitcherID)] + 1
    p19Elo$botBattersRV[which(p19Elo$player_id == pitcherID)] <- 
      p19Elo$botBattersRV[which(p19Elo$player_id == pitcherID)] + RE19$Runs_Value[row]
  }
...
b19Elo_mod$topPitchersRV <- b19Elo_mod$topPitchersRV / b19Elo_mod$topPitchersN
b19Elo_mod$botPitchersRV <- b19Elo_mod$botPitchersRV / b19Elo_mod$botPitchersN
p19Elo_mod$topBattersRV <- p19Elo_mod$topBattersRV / p19Elo_mod$topBattersN
p19Elo_mod$botBattersRV <- p19Elo_mod$botBattersRV / p19Elo_mod$botBattersN
...
b19Elo_dispID <- b19Elo_mod %>%
  unite("Name", First_Name, Last_Name, sep = " ") %>%
  mutate(Trend = round(playerElo - Trend),
         playerElo = round(playerElo + 1000 - mean(playerElo))) %>%
  select(Name, player_id, Team, Position, PA, EV, "HH%", wOBA, xwOBA, Trend, playerElo)
p19Elo_dispID <- p19Elo_mod %>%
  unite("Name", First_Name, Last_Name, sep = " ") %>%
  mutate(Trend = round(playerElo - Trend),
         playerElo = round(playerElo + 1000 - mean(playerElo))) %>%
  select(Name, player_id, Team, Position, PA, EV, "HH%", wOBA, xwOBA, Trend, playerElo)
...
write_csv(b19Elo_dispID, "data/b19Elo_dispID.csv")
write_csv(p19Elo_dispID, "data/p19Elo_dispID.csv")

# # Comparing Christian Walker and Paul Goldschmidt
# REWalker <- RE19 %>%
#   filter(Batter_MLB_ID == '572233')
# mean(REWalker$Runs_Value)
# walkerRunners <- as.data.frame(sort(table(REWalker$State_Before), decreasing=TRUE)) %>%
#   rename(Walker = Freq,
#          State = Var1)
# 
# REGoldy <- RE19 %>%
#   filter(Batter_MLB_ID == '502671')
# mean(REGoldy$Runs_Value)
# goldyRunners <- as.data.frame(sort(table(REGoldy$State_Before), decreasing=TRUE)) %>%
#   rename(Goldschmidt = Freq,
#          State = Var1) %>%
#   left_join(walkerRunners, by = 'State')
# 
# comparison <- melt(goldyRunners, id.vars = "State", measure.vars = c("Goldschmidt", "Walker")) %>%
#   rename(Batter = variable,
#          Freq = value)
# comparison$runners <- c(0, 0, 0, 1, 1, 1, 1, 1, 1, 2, 2, 1, 2, 1, 2, 2, 3, 2, 3, 2, 1, 3, 2, 2,
#                         0, 0, 0, 1, 1, 1, 1, 1, 1, 2, 2, 1, 2, 1, 2, 2, 3, 2, 3, 2, 1, 3, 2, 2)
# comparison$totRunners <- comparison$runners * comparison$Freq
# comparison %>%
#   group_by(Batter) %>%
#   na.omit() %>%
#   summarise(sum(totRunners))
# 
# ggplot(data = comparison, aes(x=State, y = Freq, group = Batter)) +
#   geom_line(aes(linetype=Batter, colour = Batter)) +
#   geom_point()
# 
# summary(b19Elo_disp$playerElo)
# summary(p19Elo_disp$playerElo)
