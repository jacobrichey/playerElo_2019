...
b19Elo_dispPS <- read_csv("data/b19Elo_dispPS.csv")
p19Elo_dispPS <- read_csv("data/p19Elo_dispPS.csv")
teamElo19 <- read_csv("data/teamElo19.csv")
teamElo19PS <- read_csv("data/teamElo19PS.csv")
...
ui <- fluidPage(
  ...
  tags$em(tags$h5("playerElo model created by Jacob Richey, Professor Abraham Wyner | University of Pennsylvania")),
  tags$em(tags$h5("Updated to end of season, including playoff baseball.")),
  tabsetPanel(
    ...
    tabPanel("Postseason playerElo Ranks", 
             tags$em(tags$h6("Note: Postseason at-bats are weighted five times as heavily as regular season at bats.")),
             tags$em(tags$h6("PostTrend = Difference in playerElo from regular season and postseason")),
             tags$p(tags$em(tags$h6("Colors are on a 5% quantile basis"))),
             fluidRow(
               column(
                 4, 
               ...
               column(
                 ...
                 selectInput("teamPS", 
                             "Team", 
                             c("All", "Atlanta Braves", "Houston Astros", 
               ...
               column(
                 ...
                 selectInput("series",
                             "Series",
                             c("World Series", "Championship Series", "Division Series", 
                               "Wild Card"), 
                             selected = "World Series"))),
             DT::dataTableOutput("postseasonElo"))
  )
)
...
server <- function(input, output) {
  ...
  output$postseasonElo <- DT::renderDataTable(DT::datatable({
    data <- b19Elo_dispPS
    if (input$posPS == "Batters") {
      data <- b19Elo_dispPS
    } else if (input$posPS == "Pitchers") {
      data <- p19Elo_dispPS
    } else if (input$posPS %in% c("C", "1B", "2B", "SS", "3B", "OF", "DH")) {
      data <- b19Elo_dispPS[b19Elo_dispPS$Position == input$posPS, ]
    } else {
      data <- p19Elo_dispPS[p19Elo_dispPS$Position == input$posPS, ]
    }
    if (input$teamPS != "All") {
      data <- data[data$Team == input$team, ]
    }
    if (input$series == "World Series") {
      data <- filter(data, Team %in% c("Houston Astros", "Washington Nationals"))
    } else if (input$series == "Championship Series") {
      data <- filter(data, Team %in% c("Houston Astros", "Washington Nationals",
                                       "New York Yankees", "St. Louis Cardinals"))
    } else if (input$series == "Division Series") {
      data <- filter(data, Team %in% c("Houston Astros", "Washington Nationals",
                                       "New York Yankees", "St. Louis Cardinals",
